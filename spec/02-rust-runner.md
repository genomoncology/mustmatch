...
`mustmatch test` runs executable Markdown through the Rust CLI. This contract invokes the binary through Cargo and asks it to run generic documentation fixtures.

## Run a documentation fixture

A documentation directory can mix bash assertion blocks, console examples, named runs, expected exits, contexts, and ordinary documentation-only fences. The runner reports supported blocks as passing and leaves documentation-only fences out of the executed case list.

```bash
cargo run -q -p mustmatch-cli --bin mustmatch -- test -v ../tests/fixtures/rust-runner | mustmatch like "PASS Bash assertion blocks run
...
The same happy-path fixture includes a documentation-only fenced example. Runnable examples appear as passes; documentation-only examples do not appear as skipped cases in the user-visible output.
...
```bash
cargo run -q -p mustmatch-cli --bin mustmatch -- test -v ../tests/fixtures/rust-runner-pyproject | mustmatch like "PASS Pyproject contexts still work
passed"
```
...
```bash
cargo run -q -p mustmatch-cli --bin mustmatch -- test -v ../tests/fixtures/rust-runner-lifecycle-pyproject/setup-hooks.md | mustmatch like "PASS Pyproject suite setup runs
...
```bash
cargo run -q -p mustmatch-cli --bin mustmatch -- test -v ../tests/fixtures/rust-runner/table-scenarios.md | mustmatch like "PASS Bash each_row substitutes table columns
[double-two]
[double-three]
...
```bash
cargo run -q -p mustmatch-cli --bin mustmatch -- test -v ../tests/fixtures/rust-runner/embedded-files.md | mustmatch like "PASS Embedded JSON files become local inputs
...
```bash
cargo run -q -p mustmatch-cli --bin mustmatch -- test -v ../tests/fixtures/rust-runner/embedded-files.md 2>&1 | mustmatch not like "FAIL
failed"
```
...
```bash
cargo run -q -p mustmatch-cli --bin mustmatch -- test -v ../tests/fixtures/rust-runner-lifecycle/setup-hooks.md | mustmatch like "PASS Suite setup runs before document blocks
...
```bash
cargo run -q -p mustmatch-cli --bin mustmatch -- test -v ../tests/fixtures/rust-runner-lifecycle/context-teardown.md | mustmatch like "PASS Context body can use setup state
...
```bash
cargo run -q -p mustmatch-cli --bin mustmatch -- test ../tests/fixtures/rust-runner-lifecycle/after-run-teardown.md >/dev/null
