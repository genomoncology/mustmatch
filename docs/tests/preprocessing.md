...
| `--collapse-whitespace` | Collapse whitespace runs to single space |
...
Use `--replace 'REGEX=>REPLACEMENT'` for dynamic values:
...
echo "key=secret1 pass=secret2" | outmatch --redact 'secret\d' "key=<redacted> pass=<redacted>"
echo "auth: Bearer eyJhbGciOiJIUzI1NiJ9" | outmatch --redact 'Bearer \S+' --contains "<redacted>"
```
